...
function Cart() {
  ...
  async function sendOrder(carId){
    const Obj = {carId , userId:localStorage.getItem('user')}
    const res = await axios.post('http://localhost:3000')
    if (res.data.succes) {
      alert("car(s) purchased")
    }else{
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {cart.map(carId => (
              ...
              <div className="w-full sm:w-48 h-32 bg-sky-100 rounded-2xl overflow-hidden">
                <div className="w-full h-full flex items-center justify-center text-blue-300 font-bold"></div>
              </div>
              <div className="flex-1 text-center sm:text-left">
                <h3 className="text-xl font-bold text-slate-900 leading-tight"></h3>
                <p className="text-blue-600 font-medium"></p>
              </div>
              <button onClick={()=>remove(carId)} className="p-3 text-slate-400 hover:text-red-500 transition-colors bg-slate-50 rounded-xl hover:bg-red-50">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-[2.5rem] p-8 shadow-xl shadow-blue-900/5 border border-white sticky top-10">
              ...
              <div className="space-y-4">
                <button className="w-full bg-blue-600 cursor-pointer hover:bg-blue-700 text-white font-bold py-5 rounded-2xl shadow-lg shadow-blue-200 transition-all active:scale-95">
                  Proceed to Checkout
                </button>
              </div>
            </div>
